...
beautiful.init("/home/fbakly/.config/awesome/theme.lua")
...
function update_tray()
	tray:set_screen(awful.screen.focused())
end

function update_bat()
	awful.spawn.easy_async({"sh", "-c", "cat /sys/class/power_supply/BAT*/capacity | tr '\n' ' '"},
	function(out)
		mybattery.markup = " " .. out
	end)
end

function update_vol()
	awful.spawn.easy_async({"sh", "-c", "pamixer --get-volume"},
	function(out)
		pulse.markup = "墳 " .. out
	end)
end
...
function update_time()
	awful.spawn.easy_async({"sh", "-c", "date +\"%H:%M   %d-%m-%Y\""},
	function(out)
		mytextclock.markup = " " .. out	
	end
	)
end

function update_cpu()
	awful.spawn.easy_async({"sh", "-c", "vmstat 1 1 | awk 'NR==3 {print $13; printf \"%%\"}' | tr -d '\n'"},
	function(out)
		cpu.markup = "CPU " .. out
	end)
end
...
function update_mem()
	awful.spawn.easy_async({"sh", "-c", "free -m | awk '/Mem/ {print $3\"/\"$2 \" MB\"}'"},
	function(out)
		mymem.markup = "Mem " .. out
	end
	)
end

function update_temp()
	awful.spawn.easy_async({"sh", "-c", "expr $(cat /sys/class/thermal/thermal_zone0/temp) / 1000"},
	function(out)
		temp.markup = "﨎" .. out
	end)
end
...
function update_pacman()
	awful.spawn.easy_async({"sh", "-c", "pacman -Qu | wc -l"},
	function(out)
		pacman.markup = " " .. out
	end)
end
...
globalkeys = gears.table.join(
	...
	awful.key({ "Shift" }, "Print", function () awful.spawn.with_shell("/home/fbakly/scripts/screenshot 1") end,
			{description = "Screenshot Copy", group = "Scripts"}),
	awful.key({ modkey, "Shift" }, "b", function () awful.spawn.with_shell("/home/fbakly/scripts/bltconnect") end,
			{description = "Screenshot Copy", group = "Scripts"}),
	awful.key({ }, "XF86AudioRaiseVolume", function ()
		awful.spawn.with_shell("pamixer -u -i 5")
		update_vol()
	end,
			{description = "Increase Volume", group = "Multimedia"}),
	awful.key({ }, "XF86AudioLowerVolume", function ()
		awful.spawn.with_shell("pamixer -u -d 5")
		update_vol()
	end,
